...
const RequestGet = function(url, params) {
    return $.ajax({url: url + "?" + jQuery.param(params), type: 'GET'});
}

const lookUpPrices = async function(id_array) {
    let ids = id_array.join(",");
    return RequestGet("https://api.coingecko.com/api/v3/coins/markets", {ids: ids, "vs_currency": "usd", price_change_percentage: "1h,24h,7d"});
};
...
function prepare_data() {
    if (typeof trades === 'undefined') {
        ...
        window.trades = new_datas["trades"]
        window.coins = new_datas["coins"]
        window.balances = new_datas["balances"]
        window.addresses = new_datas["addresses"]
        window.coin_list = new_datas["coin_list"]
        window.MAIN_ADDRESS = new_datas["MAIN_ADDRESS"]
    }
    else {
        ...
        window.MAIN_ADDRESS = MAIN_ADDRESS
    }
}
